...
Our HTTP Backend for the Mosquitto Auth Plugin will use information we have about the devices, and [Django's User Authentication and Authorization system](https://docs.djangoproject.com/en/5.0/topics/auth/).
...
For non-websockets clients, we will authenticate against Django's User model.  See [authenticate](https://docs.djangoproject.com/en/5.0/topics/auth/default/#django.contrib.auth.authenticate).
...
Django users connected on port 50002 (all "regular" Django users; websockets/mqtt protected with a server SSL/TLS Certificate) should only have access to Read/Write relevant topics related to the devices they "own" (associated with).  
...
There's an asymmetric relationship between the access controls for MQTT topics for Users and their devices bridges.  For a topic like `"devices/b827eb08451e/lamp/set_config"` the User (say, via websockets/mqtt) needs to be able to Write (Publish) to the topic, but the device bridge needs to Read (Subscribe) from the topic.  **This is asymmetry in access control critically important to understand.**
