...
**NOTE:** the **mosquitto-go-auth** requires that the Mosquitto broker itself be built from source code.  We will use Ansible to properly install and configure Mosquitto and the plugin. Ansible will _remove_ the Debian/Ubuntu Mosquitto server package we have been using and then build and install a custom version of Mosquitto, along with the plugin.
...
**RUN ANSIBLE TO PROPERLY SET UP YOUR ENVIRONMENT FOR THIS ASSIGNMENT**
...
For this section, we will assume that you have the **go-auth.so** installed (you ran Ansible, right?).  Ansible will install it in `/etc/mosquitto/plugins/`.
...
Please refer to the [HTTP Backend Configuration](https://github.com/iegomez/mosquitto-go-auth?tab=readme-ov-file#http) for more information.


**NOTE:** this disallows "anonymous" connections - all connections must be authenticated with username/password (or clients must use a SSL/TLS certificate, like our LAMPI bridge).
...
`tmux` can be a useful tool - you can use it or just open up multiple SSH terminal sessions in separate windows.
